...
+(BOOL) hasInternet{
    ...
    NSString* netTyep = [self getInternetType];
    if (netTyep == nil) {
        return false;
    }
...
+(BOOL) hasWifi{
    ...
    NSString* netTyep = [self getInternetType];
    if ( netTyep!= nil && [netTyep isEqualToString:@"wifi"]) {
        return true;
    }
    return false;
}

+(NSString*) getInternetType{
    NSString* result;
...
+(NSString*) getVersionName{
    NSString* versionName = [@"IOS:" stringByAppendingString:[self getAppVersion]];
    return versionName;
}
